...
<span class="text-tuio-navy font-bold">${productName}</span> </div> <div class="grid md:grid-cols-2 gap-12 bg-white rounded-[32px] p-8 shadow-sm"> <!-- Image Gallery --> <div class="space-y-4"> <div class="aspect-square bg-gray-100 rounded-2xl overflow-hidden border border-gray-200 p-8 flex items-center justify-center relative group"> <img${addAttribute(Array.isArray(displayProduct.images) && displayProduct.images.length > 0 ? displayProduct.images[0] : displayProduct.image_url || "/images/pattern.png", "src")}${addAttribute(productName, "alt")} class="max-w-full max-h-full object-contain mix-blend-multiply group-hover:scale-110 transition-transform duration-500"> <div class="absolute top-4 left-4 bg-tuio-red text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider"> ${displayProduct.category} </div> </div> </div> <!-- Product Info --> <div> <div class="flex items-center justify-between mb-4"> <div class="flex items-center gap-2 text-yellow-500"> ${"\u2605".repeat(
    Math.round(
...
Product
</label> <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100"> <img${addAttribute(Array.isArray(displayProduct.images) && displayProduct.images.length > 0 ? displayProduct.images[0] : displayProduct.image_url || "/images/pattern.png", "src")} class="w-10 h-10 rounded-lg object-cover"> <span class="font-bold text-tuio-navy">${productName}</span> </div> </div> <div> <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest pl-1 mb-2">
Your Message
