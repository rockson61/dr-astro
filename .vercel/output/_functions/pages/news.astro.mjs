...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  let articles = [];
  try {
    const { data, error } = await supabase.from("articles").select("*, author:profiles(full_name, avatar_url), category:categories(name, slug)").eq("status", "published").eq("category.slug", "news").order("published_at", { ascending: false }).limit(12);
...
Dental<br> <span class="text-transparent bg-clip-text bg-gradient-to-r from-tuio-red to-red-500">News</span> </h1> <p class="text-gray-400 max-w-2xl mx-auto text-lg font-light">Stay informed with the latest developments in dental science, technology, and business.</p> </div> </section> <div class="bg-tuio-bg min-h-screen py-12"> <div class="container mx-auto px-4"> <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8"> ${articles.map((article) => renderTemplate`<article class="bg-white rounded-[32px] overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all group border border-transparent hover:border-tuio-red"> <div class="relative h-56 overflow-hidden"> <img${addAttribute(article.image_url || "/images/pattern.png", "src")}${addAttribute(article.title, "alt")} class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"> <span class="absolute top-4 left-4 bg-white/90 backdrop-blur text-tuio-navy px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider"> ${article.category?.name || "News"} </span> </div> <div class="p-8"> <div class="text-sm text-tuio-red mb-3 uppercase tracking-widest font-bold"> ${format(new Date(article.published_at), "MMM d, yyyy")} </div> <h2 class="font-tuio text-2xl uppercase text-tuio-navy mb-4 leading-tight group-hover:text-tuio-red transition-colors"> <a${addAttribute(`/articles/${article.slug}`, "href")}> ${article.title} </a> </h2> <p class="text-gray-500 line-clamp-2 mb-6 font-light">${article.excerpt}</p> <div class="flex items-center justify-between border-t border-gray-100 pt-6"> <span class="text-sm font-bold text-gray-400 uppercase tracking-widest">
