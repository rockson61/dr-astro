...
const cleanSlug = (slug: string) => decodeURIComponent(slug);
...
export const GET: APIRoute = async ({ request }) => {
    try {
        ...
        let query = supabase
            .from('articles')
            .select('id, title, excerpt, author, category, image_url, images, created_at, views_count, likes_count, user_id', { count: 'exact' })
            .eq('is_approved', true);
        ...
        // Generate slugs
        const articlesWithSlugs = (data || []).map((article: any) => ({
            ...article,
...
export const POST: APIRoute = async ({ request }) => {
    try {
        ...
        if (isNumeric) {
            ...
            // Extract ID from slug format "slug-id:123" or "slug-123" if applicable
            ...
            if (!article) {
                const decodedTitle = decodedSlug
                    .replace(/-id:\d+$/, '')
                    .replace(/-\d+$/, '')
                    .replace(/-/g, ' ')
                    .replace(/\s+/g, ' ')
